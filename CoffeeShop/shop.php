
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Shop</title>

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-BmbxuPwQa2lc/FVzBcNJ7UAyJxM6wuqIj61tLrc4wSX0szH/Ev+nYRRuWlolflfl" crossorigin="anonymous">
    <link rel="stylesheet" href="css/shop2.css">

    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans:wght@300&family=Oswald&family=Parisienne&display=swap" rel="stylesheet">

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">

    <link rel="stylesheet" href="css/animat/animate.min.css" />

  </head>
  <body>

    <div id="shop-section">
      <div class="shop">

        <nav class="navbar navbar-expand-lg">
      <div class="container-fluid">
        <h4 class="navbar-brand" href="#">Cafe</h4>
        <button class="navbar-toggler custom-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon custom-toggler"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
            <li class="nav-item">
              <a class="nav-link active buton-nav" href="index.php">About</a>
            </li>
            <li class="nav-item">
              <a class="nav-link buton-nav" href="menu.php">Menu</a>
            </li>
            <li class="nav-item">
              <a class="nav-link buton-nav focus"  aria-current="page" href="#">Shop</a>
            </li>
            <li class="nav-item">
              <a class="nav-link buton-nav" href="contact.php">Reservation</a>
            </li>
          </ul>
        </div>
      </div>
    </nav>

    <div class="title-text text-center">
      <h1 id="big-title">Shop Products</h1>
    </div>

      </div>


      <div class="products">
        <!--<div class="first-line">
          <div class="product-title">BAGGED COFFEE</div>
        </div>-->
        <div class="row line wow slideInLeft" data-wow-duration="1.5s">
          <div class="first-line">
            <div class="product-title">BAGGED COFFEE</div>
          </div>
          <div class="pricing-column col-lg-4 col-md-12">
            <div class="card text-center">
              <div class="card-body">
                <a href="#">
                <img src="images/coffeePack.jpg" alt="donut.jpg" height="300rem" class="image">
                <div class="middle2">
                  <div class="hidden-text">Order</div>
                </div>
                </a>
              </div>
              <div class="card-footer">
                <h4>Mocha Java</h4>
                <p>$19.99</p>
              </div>
            </div>
          </div>

          <div class="pricing-column col-lg-4 col-md-12">
            <div class="card text-center">
              <div class="card-body">
                <a href="#">
                <img src="images/coffeePack.jpg" alt="donut.jpg" height="300rem" class="image">
                <div class="middle2">
                  <div class="hidden-text">Order</div>
                </div>
              </a>
              </div>
              <div class="card-footer">
                <h4>Colombian</h4>
                <p>$19.99</p>
              </div>
            </div>
          </div>

          <div class="pricing-column col-lg-4 col-md-12">
            <div class="card text-center">
              <div class="card-body">
                <a href="#">
                <img src="images/coffeePack.jpg" alt="donut.jpg" height="300rem" class="image">
                <div class="middle2">
                  <div class="hidden-text">Order</div>
                </div>
              </a>
              </div>
              <div class="card-footer">
                <h4>Honduras</h4>
                <p>$19.99</p>
              </div>
            </div>
          </div>
        </div>


        <div class="row line wow slideInLeft" data-wow-duration="1.5s">
          <div class="first-line">
            <div class="product-title">K CUPS</div>
          </div>
          <div class="pricing-column col-lg-4 col-md-12">
            <div class="card text-center">
              <div class="card-body">
                <a href="#">
                <img src="images/kCup2.png" alt="donut.jpg" height="300rem" class="image">
                <div class="middle">
                  <div class="hidden-text">Order</div>
                </div>
              </div>
            </a>
              <div class="card-footer">
                <h4>Colombian</h4>
                <p>K Cup</p>
                <p>$12.95</p>
              </div>
            </div>
          </div>

          <div class="pricing-column col-lg-4 col-md-12">
            <div class="card text-center">
              <div class="card-body">
                <a href="#">
                <img src="images/kCup2.png" alt="donut.jpg" height="300rem" class="image">
                <div class="middle">
                  <div class="hidden-text">Order</div>
                </div>
              </div>
            </a>
              <div class="card-footer">
                <h4>Ethiopian</h4>
                <p>K Cup</p>
                <p>$12.95</p>
              </div>
            </div>
          </div>
        </div>


        <div class="row line wow slideInLeft" data-wow-duration="1.5s">
          <div class="first-line">
            <div class="product-title">MUGS</div>
          </div>
          <div class="pricing-column col-lg-4 col-md-12">
            <div class="card text-center">
              <div class="card-body">
                <a href="#">
                <img src="images/blackCup.png" alt="donut.jpg" height="300rem" class="image">
                <div class="middle3">
                  <div class="hidden-text">Order</div>
                </div>
              </a>
              </div>
              <div class="card-footer">
                <h4>Original Mug</h4>
                <p>Black</p>
                <p>$14.95</p>
              </div>
            </div>
          </div>

          <div class="pricing-column col-lg-4 col-md-12">
            <div class="card text-center">
              <div class="card-body">
                <a href="#">
                <img src="images/whiteCup.png" alt="donut.jpg" height="300rem" class="image">
                <div class="middle3">
                  <div class="hidden-text">Order</div>
                </div>
              </a>
              </div>
              <div class="card-footer">
                <h4>Original Mug</h4>
                <p>White</p>
                <p>$14.95</p>
              </div>
            </div>
          </div>
        </div>


      </div>
    </div>

    <script type="text/javascript" src="js/jquery/jquery.js"></script>

    <script type="text/javascript" src="js/script.js"></script>

    <script type="text/javascript" src="js/bootstrap/bootstrap.min.js"></script>

    <script type="text/javascript" src="js/wow/wow.min.js"></script>

  </body>


  <footer class="container-fluid bg-grey py-5">

    <div class="container">
   <div class="row">
      <div class="col-md-6">
         <div class="row">
            <div class="col-md-6 ">
               <div class="logo-part">
                 <h1 class="footer-logo">Cafe</h1>
                  <!--<img src="https://i.ibb.co/sHZz13b/logo.png" class="w-50 logo-footer" >-->

                  <p>7637 Laurel Dr. King Of Prussia, PA 19406</p>
                  <p>Use this tool as test data for an automated system or find your next pen</p>
               </div>
            </div>
            <div class="col-md-6 px-4">
               <h6> About Company</h6>
               <p>But horizontal lines can only be a full pixel high.</p>
               <a href="#" class="btn-footer"> More Info </a><br>
               <a href="#" class="btn-footer"> Contact Us</a>
            </div>
         </div>
      </div>
      <div class="col-md-6">
         <div class="row">
            <div class="col-md-6 px-4">
               <h6> Help us</h6>
               <div class="row ">
                  <div class="col-md-6">
                     <ul>
                        <li> <a href="#"> Home</a> </li>
                        <li> <a href="#"> About</a> </li>
                        <li> <a href="#"> Service</a> </li>
                        <li> <a href="#"> Team</a> </li>
                        <li> <a href="#"> Help</a> </li>
                        <li> <a href="#"> Contact</a> </li>
                     </ul>
                  </div>
                  <div class="col-md-6 px-4">
                     <ul>
                        <li> <a href="#"> Cab Faciliy</a> </li>
                        <li> <a href="#"> Fax</a> </li>
                        <li> <a href="#"> Terms</a> </li>
                        <li> <a href="#"> Policy</a> </li>
                        <li> <a href="#"> Refunds</a> </li>
                        <li> <a href="#"> Paypal</a> </li>
                     </ul>
                  </div>
               </div>
            </div>
            <div class="col-md-6 ">
               <h6> Newsletter</h6>
               <div class="social">
                  <a href="#"><i class="fa fa-facebook" aria-hidden="true"></i></a>
                  <a href="#"><i class="fa fa-instagram" aria-hidden="true"></i></a>
               </div>
               <form class="form-footer my-3">
                  <input type="text"  placeholder="Enter your Email Address" name="search">
                  <input type="button" value="Go" >
               </form>
               <p>© Copyright 2021 Magherusan Oana</p>
            </div>
         </div>
      </div>
   </div>
</div>
</div>


  </footer>
</html>
